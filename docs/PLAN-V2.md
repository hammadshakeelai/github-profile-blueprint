...
> **Outcome:** `profile/` — the README plus fourteen SVGs, generated by
> `tools/profile/build.py` from the same `content.json` as the comps, and
> verified on GitHub through a staging copy: 7/7 images load, smallest text
> 11.9px at phone width, one column, both schemes. Written up in
> `docs/PROFILE.md`, which also carries the push commands, since this session's
> credentials can't write to `hammadshakeelAl`.
