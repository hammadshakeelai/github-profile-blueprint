...
# GitHub's CSP blocks *loading* an external font for SVGs referenced as images,
# so @font-face and web-font URLs never arrive. It does not stop a font the
# viewer already has installed. So 'JetBrains Mono' stays first — anyone with it
# locally gets the intended face — backed by system stacks that resolve to a real
# font everywhere else, instead of falling through to an unstyled default.
#
# Note 'ui-monospace' / 'SFMono-Regular' are the real CSS tokens here;
# '-apple-system-ui-monospace' is not a valid keyword and is silently ignored.
SANS_STACK = ("-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, "
              "'Helvetica Neue', Arial, sans-serif")
MONO_STACK = ("'JetBrains Mono', ui-monospace, SFMono-Regular, Menlo, Consolas, "
...
def render_svg_dashboard(config: dict, metrics: dict):
  ...
  <defs>
    <style>
      ...
      .header {{ font-family: {MONO_STACK}; font-size: 13px; fill: #38bdf8; font-weight: bold; }}
      .label {{ font-family: {SANS_STACK}; font-size: 11px; fill: #94a3b8; }}
      .value {{ font-family: {MONO_STACK}; font-size: 18px; font-weight: bold; fill: #f8fafc; }}
  ...
  <text x="24" y="120" font-family="{MONO_STACK}" font-size="10" fill="#64748b">SYNCHRONIZED: {sync_time} UTC // GIT-NATIVE ZERO CAMO LATENCY</text>
</svg>"""
...
def render_quantum_coherence_svg(config: dict, metrics: dict):
  ...
  <defs>
    <style>
      ...
      .hud-title {{ font-family: {MONO_STACK}; font-size: 12px; fill: #38bdf8; font-weight: bold; letter-spacing: 1px; }}
      .hud-val {{ font-family: {MONO_STACK}; font-size: 15px; fill: #f8fafc; font-weight: bold; }}
      .hud-sub {{ font-family: {MONO_STACK}; font-size: 10px; fill: #64748b; }}
