...
for (const engine of ['chromium', 'firefox', 'webkit']) {
  ...
  const p = await (await b.newContext({ viewport: { width: 1280, height: 9000 } })).newPage();
...
const fmt = r => !r ? '—' : !r.decodes ? 'NO DECODE' : [r.lit === undefined ? '' : (r.lit ? 'lit' : 'NOT LIT'), r.animates ? 'moves' : 'still'].filter(Boolean).join(' · ');
...
fs.writeFileSync('docs/frontier/data/probe-github.json', JSON.stringify(out, null, 1));
